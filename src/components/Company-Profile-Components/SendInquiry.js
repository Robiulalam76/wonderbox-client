...
const SendInquiry = ({ product }) => {
  ...
  return (
    <section id="send-inquiry" className="my-16 min-h-full relative">
      <h1 className="text-black font-semibold text-xl text-left my-4">
        Send Inquiry{" "}
      </h1>
      <div className="flex items-center gap-3 mt-4">
        ...
        <p className="md:text-xl font-semibold text-black text-left">
          {store?.name}
        </p>
      </div>
      <div className="flex items-center gap-2 mt-4">
        ...
        <div className="w-5 h-5 rounded-full border p-1 flex justify-center items-center">
          <span className="text-black font-bold">!</span>
        </div>
      </div>

      <p className="text-gray-500 text-left my-4">
      ...
      <textarea
        ...
        className="border resize-none border-[#0029FF4D] focus:outline-primary rounded-md h-72 w-full p-6"
        name="message"
      ...
      <div className="flex items-center gap-4 mt-4">
        ...
        <p className="text-gray-500 text-left">
          Check out this question from other buyers. Click to insert the
      ...
      <div className="flex flex-col justify-center md:justify-start md:flex-row md:items-center gap-5 mt-6">
        <button
          onClick={() => setQuestion1(question1 ? "" : questions[0])}
        ...
        <button
          onClick={() => setQuestion2(question2 ? "" : questions[1])}
          className={`w-full md:w-96 h-12 flex justify-center items-center rounded-[50px]
                    ${
        ...
        <button
          onClick={() => setQuestion3(question3 ? "" : questions[2])}
          className={`w-full md:w-96 h-12 flex justify-center items-center rounded-[50px]
                    ${
      ...
      <div className="relative w-56 h-12 flex justify-center cursor-pointer items-center gap-3 border border-primary rounded-md mt-4">
        <img className="w-5" src={attach} alt="" />
      ...
      <button className="flex items-center gap-3 mt-6">
        ...
        <span className="text-sm text-left text-gray-500">
          I agree to share my{" "}
          <strong className="text-gray-900">Business Card</strong> to the
          supplier
        </span>
      </button>
      ...
      <button
        ...
        className="w-56 h-12 bg-primary flex justify-center items-center rounded-md mx-auto my-14"
        type="submit"
      >
      ...
      {/* <div className='fixed bottom-0 right-0 w-full md:w-fit h-fit bg-white'>
                <div className={`flex justify-between items-center w-full md:w-96 h-16 bg-white shadow border border-gray-100 px-4 ${showMessage && 'shadow-xl shadow-b'}`}>
                    ...
                    <div className='flex items-center gap-5'>
                        ...
                        <div>
                            {
                                showMessage ? <img onClick={() => setShowMessage(false)} className='w-4 cursor-pointer' src={close} alt="" />
                                    :
                                    <img onClick={() => setShowMessage(true)} className='w-4 cursor-pointer' src={downArrow} alt="" />
                            }
                        </div>
                    </div>
                </div>
