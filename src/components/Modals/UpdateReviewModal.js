...
const UpdateReviewModal = ({ open, setOpen, refetch }) => {
  ...
  const [isPositive, setIsPositive] = useState(
    open?.isPositive ? "Positive" : "Nagetive"
  );
  ...
  const sendReview = (data) => {
    ...
    fetch(`${process.env.REACT_APP_API_KEY}/api/review/update/${open?._id}`, {
      method: "PUT",
      headers: {
          ...
          <div className="grid grid-cols-1 gap-4 max-w-[600px] h-fit bg-white rounded-md border p-4 mt-6">
            <div>
              ...
              <Typography className="font-normal text-gray-500 text-sm">
                I recently purchased this product and I am extremely satisfied
                with its performance. It exceeded my expectations in terms of
                quality and functionality. Highly recommended!
              </Typography>
            </div>
